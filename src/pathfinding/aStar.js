...
const findPath = (start, end, delay) => {
    ...
    while (openSet.size > 0) {
        ...
        for (let neighbour of neighbours) {
            ...
            if (closedSet.has(neighbour.toString())) continue;

            operationCount++;

            let tentativeGScore = gScore[current] + 10;
            const neighboursGScore = gScore[neighbour];

            if (tentativeGScore < neighboursGScore) {
                cameFrom[neighbour.toString()] = current.toString();
                gScore[neighbour.toString()] = tentativeGScore;
                fScore[neighbour.toString()] = gScore[neighbour] + getHCost(neighbour, end);
                if (!openSet.has(neighbour.toString())) {
                    openSet.add(neighbour.toString());
                }
            }
