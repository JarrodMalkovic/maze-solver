...
const bidirectional = (grid, delay = 10) => {
    ...
    if (!startNode || !endNode) {
        ...
        if (Object.keys(path[0]).length > 0) {
            ...
            const totalPath = startConstructedPath.concat(endConstructedPath.reverse());
...
const findPath = (start, end, delay) => {
    ...
    while (startQueue.length && endQueue.length) {
        ...
        if (endNodeSet.has(startNodeFrontier.join('-')) || startNodeFrontier.join('-') === end.join('-')) {
            return [
        ...
        } else if (startNodeSet.has(endNodeFrontier.join('-')) || endNodeFrontier.join('-') === start.join('-')) {
            return [
        ...
        endOperationCount += BFS(endNodeFrontier, endQueue, endNodeSet, endNodePath, endCameFrom, endOperationCount);
    }
